...
Download these pretrained models and place in `pretrained_models/`:
...
### HybrIK-X Model (Optional - for SMPL-X)
```bash
# Download model weights
...
```
pretrained_models/
├── hybrik_hrnet.pth          # Main HybrIK model (required)
└── hybrikx_rle_hrnet.pth     # HybrIK-X model (optional)

configs/
├── 256x192_adam_lr1e-3-hrw48_cam_2x_w_pw3d_3dhp.yaml
└── smplx/256x192_hrnet_rle_smplx_kid.yaml
```